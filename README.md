...
- **归因公式**（来自接口文档，逐股聚合）：
  ...
  - `持仓 P&L = shares_prev × (close − pre_close)`
  - `买入 P&L = close × buy_shares − buy_amount`（当日买入 → 收盘价的差）
  - `卖出 P&L = sell_amount − pre_close × sell_shares`（昨收 → 当日卖出价的差）
...
- **明细表**：股票代码（含「停牌」「已清」Tag）、槽位（数组，多 Tag 并列）、贡献度、贡献金额（hover 展开「持仓 / 买入 / 卖出」三段 P&L 计算式）、涨跌幅、涨跌额、收盘、昨收、当日持股 / 开盘前持股、当日交易（买/卖 Tag）、交易成本（hover 展开佣金/印花税/滑点）
- **特殊状态处理**：
  - **停牌**：`close/pre_close` 为 `null`，贡献金额计为 0，列内 Tag 标记
  - **当日完全卖出**：执行回放后不在 `holdings/daily.items` 但出现在当日 executions 的股票，单独追加一行（粗略用 `sell_amount − buy_amount` 估算 P&L，因缺 `pre_close` 不精确，UI 标「已清」）
